...
def cmd_status(args):
    ...
    sigs = conn.execute(
        ...
        " FROM signatures ORDER BY priority_score DESC LIMIT 10").fetchall()
    print("== signatures ==")
    for s in sigs:
        print(f"  {s['id']}  {s['volume_7d']}/wk  ${s['avg_cost_usd']:.5f}  "
    ...
    print("\n== skills ==")
    for k in skills:
        print(f"  {k['signature_id']}  v{k['version']}  [{k['stage']}]  {k['samples']} samples")
