...
def filter_by_hardware(pool: list[BaseCandidate], hw: Hardware,
                       serving: bool = False) -> tuple[list[BaseCandidate], list[str]]:
    reasons: list[str] = []
    if hw.kind == "cpu_only":
        return [], ["no GPU detected — distiller disabled; "
...
def score_candidate(c: BaseCandidate, sigs: list[SignatureProfile],
                    existing_base_wins: dict[str, int],
                    all_signatures_count: int) -> float:
...
def select_base(sig_profiles: list[SignatureProfile],
                existing_base_wins: dict[str, int],
                hw: Optional[Hardware] = None,
                pool: Optional[list[BaseCandidate]] = None) -> Selection:
    hw = hw or detect_hardware()
    pool = pool or load_pool()

    if hw.kind == "cpu_only":
        return Selection(
            candidate=pool[0] if pool else DEFAULT_POOL[0], score=0.0,
    ...
    if not fit:
        return Selection(
            candidate=pool[0], score=0.0, distiller_enabled=False,
    ...
    return Selection(candidate=top, score=top_score, runner_up=runner,
                     reasons=[r for r in
                              [f"tier={top.tier}", f"{top.tokens_per_sec_gpu} tok/s"]])


def replace_backend(c: BaseCandidate, hf_id: str, backend: str) -> BaseCandidate:
    return BaseCandidate(c.name, c.tier, c.vram_gb_qlora * 0.55,  # 4-bit MLX smaller
                         c.vram_gb_serve * 0.55, c.tokens_per_sec_gpu * 0.8,
                         backend, hf_id)
