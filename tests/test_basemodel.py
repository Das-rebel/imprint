...
def test_pool_loads_with_all_tiers():
    tiers = {c.tier for c in POOL}
    assert tiers == {"small", "medium", "large"}


def test_nvidia_16gb_filters_large_models():
...
def test_cpu_only_disables_distiller():
...
def test_apple_silicon_swaps_to_mlx():
...
def test_consolidation_bonus_prefers_shared_base():
...
def test_low_economics_gates_out_training():
